...
- [x] **Harden solver helpers against conflicting givens** — **DONE 2026-07-08**. `countSolutions()`/`solve()` in [generator.js](../js/generator.js) now guard themselves with `hasConflictingGivens()` at their public entry point (the recursive search was split into a private `countSolutionsRec` helper so the check runs once, not on every recursive step) — `countSolutions()` returns `0` and `solve()` returns `null` for a board whose givens already conflict, regardless of caller. `app.js`'s custom-entry validation still calls `hasConflictingGivens()` first for its more specific error message, which is now a UX nicety rather than a correctness requirement. Added a committed regression suite at [tests/generator.test.js](../tests/generator.test.js) (run with `node tests/generator.test.js`, no framework/deps) — 9/9 checks pass, covering the conflicting-givens guard plus non-regression of normal solving/uniqueness behavior.
- [x] **Delete dead `js/worker.js`** — **DONE 2026-07-08**. File removed; also dropped from the SW `ASSETS` precache list ([sw.js](../sw.js)) and from the file-tree/notes in [.claude/plan.md](plan.md) (both previously said to keep it since the SW cached it). Puzzle generation still runs on the main thread via a double `requestAnimationFrame()` in `startNewGame()` ([app.js](../js/app.js) ~line 60). Revisit off-thread generation later if generation time becomes a real problem (worth re-measuring after the puzzle-catalogue feature lands, since that may replace live generation entirely) — build fresh against `generateGraded()` rather than resurrecting this file.
- [x] **Service worker fetch handler should enforce same-origin** — **DONE 2026-07-08**. Added `if (new URL(e.request.url).origin !== location.origin) return;` to the fetch handler in [sw.js](../sw.js), matching the pre-existing comment. No cache-version bump needed — `sw.js` itself isn't SW-cached, so the browser always fetches it fresh.
...
- [x] **Explain solving techniques in Help** — **DONE 2026-07-08**. Added a "Solving Techniques" section to the Help dialog ([index.html](../index.html)) with a `<dl class="technique-list">` covering all 11 techniques (Naked/Hidden Single, Pointing Pair/Triple, Box-Line Reduction, Naked Pair, Hidden Pair, Naked Triple, X-Wing, Swordfish, XY-Wing, Unique Rectangle), each with a short player-friendly explanation. An intro line connects it to the hint-chain UI: purple = pattern cells, muted purple = eliminations, ‹ › steps through multi-step chains. New CSS in [style.css](../css/style.css) reuses existing `--given-color`/`--note-color` (already theme-aware). Verified in a real headless-Chromium browser: dialog opens, section renders and scrolls correctly, all 11 entries present, 0 console errors. SW cache bumped v42→v43.

- [ ] **Add Tutorial Mode for hint explanations** — add a setting/toggle named "Tutorial Mode" for hinted-move explanations, default off. When enabled and a hint chain step is shown, provide an explanation that explicitly states why the selected number can be placed or why highlighted candidates can be removed. The explanation should use the current step data (`type`, `patternCells`, `eliminations`, `placement`, and relevant unit/peer relationships) rather than generic text, so players can connect the rule to the visible board.
...
- Service worker cache is currently `sudoku-v43`.
